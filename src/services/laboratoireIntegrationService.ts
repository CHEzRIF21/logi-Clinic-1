...
export class LaboratoireIntegrationService {
  ...
  /**
   * Vérifie le statut de paiement d'une prescription
   */
  static async checkPaiementStatus(prescriptionId: string): Promise<{
    peut_prelever: boolean;
    message: string;
    ...
    try {
      const { data: prescription, error } = await supabase
        .from('lab_prescriptions')
        .select('statut_paiement, montant_total, facture_id, ticket_facturation_id')
        .eq('id', prescriptionId)
        .single();
      ...
      // Si le paiement est obligatoire (configurable)
      const { data: config } = await supabase
        .from('configurations_laboratoire')
        .select('valeur')
        .eq('cle', 'labo_paiement_obligatoire')
      ...
      if (!paiementObligatoire) {
        return {
          peut_prelever: true,
          message: 'Paiement non obligatoire',
          statut_paiement: prescription.statut_paiement
        };
      }

      // Vérifier le statut de paiement
      if (prescription.statut_paiement === 'paye') {
        return {
          peut_prelever: true,
          message: 'Paiement effectué',
          statut_paiement: prescription.statut_paiement
      ...
      if (prescription.statut_paiement === 'partiel') {
        return {
          peut_prelever: false,
          message: 'Paiement partiel - montant restant à payer',
          statut_paiement: prescription.statut_paiement,
          montant_restant: prescription.montant_total || 0
        };
      }

      return {
        peut_prelever: false,
        message: `Paiement requis: ${prescription.montant_total || 0} XOF`,
        statut_paiement: prescription.statut_paiement,
        montant_restant: prescription.montant_total || 0
      };
    } catch (error) {
